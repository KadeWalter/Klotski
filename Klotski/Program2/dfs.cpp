...
//Finds the second empty space on the board and tries to move the highest numbered block into it.. repeats and if it cant move a block to that space, it tries again with the other blank space
//By Kade Walter
//
...
void dfs::dfsMain(string name)
{
	ofstream outFile(name + "DFS.out");
	//print the initial state (0 moves)
	dfs::print(outFile);
	dfs::solver(outFile);

	outFile << "This solution has " << co - 1 << " steps to solve." << endl;
	outFile.close();
}

void dfs::solver(ofstream &outFile)
{
	string lastUsed;
	bool solution = false;
...
bool dfs::moveUp(int row, int col)
{
	int pieceSize = 1;
...
bool dfs::moveDown(int row, int col)
{
	int pieceSize = 1;
	string piece = GAME_BOARD[row][col];
	//if the piece is the winning piece
...
bool dfs::moveLeft(int row, int col)
{
	int pieceSize = 1;
	string piece = GAME_BOARD[row][col];
	//if the piece is the winning piece
...
bool dfs::moveRight(int row, int col)
{
	int pieceSize = 1;
	string piece = GAME_BOARD[row][col];
	//if the piece is the winning piece
...
void dfs::print(ofstream &outFile)
{
